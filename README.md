...
<img src="man/figures/README-LIMONapproach.png" width="100%" />

## Installation

You can install the development version of LIMON using devtools:
...
The following is an example impliminatation of running LIMON using data
...
samples collected longitduinaly from 150 pre-term infants. 30 of these
...
to pull the relative abundance metagenomic tables for the 30 infants
with NEC and healthy infants who did not develop NEC.

<br> Recommended tips for this procedure are;
...
- Perform model selection as outlined in
...
days time point 2, and 18-26 days is timepoint 3.
...
Now that we have cleaned meta data, clean up the relative abundance
count tables. For this dataset, we filtered to the top 50 most prevelent
taxa. Note this dataset provides the counts in terms of proportional
data.
...
Diet (breast milk vs formula), birth mode (cesarean vs vaginal) and
birth weight (grams).
...
#### 1f: SPIEC-EASI per time

For information on choosing the appropriate SPIEC EASI parameters,
please see their [documentation](https://github.com/zdk123/SpiecEasi).
For this example we used “bstars” as the selection criteria.
...
To check network stability, use the getStability() function to see if it
