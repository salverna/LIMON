                           ...
                           distribution = "LMM", correlation = NULL)
{
  ...
  for (col_name in colnames(Obj$Counts)) {
    ...
    # equation for other models
    eq <- formula(paste("asvcounts", "~", model))

    # Zero Inflated Gaussian Mixed Model
    if (distribution == "LME.Z") {
      # Check if each column contains at least one zero
      cols_without_zeros <- which(!apply(Obj$Counts, 2, function(col) any(col == 0)))
    ...
    # Extract the intercept and covariate values
    if (distribution == "LMM") {
    ...
    # Subtract beta * covariate for each covariate
    for (covariate in Covariates) {
  ...
  # Add the corrected values to a dataframe
  ...
  for (tp in timepoints) {
    ...
    timept_counts_name <- paste0("Corrected_Counts_",
                                 tp)
